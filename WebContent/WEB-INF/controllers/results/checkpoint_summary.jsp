...
function addSelectedText(addId, removeId, row, col, num) {
    var key = row + '-' + col;
    var text = $('#' + row + '-' + col + ' > div:nth(' + num + ') > input').val();
    $('#' + row + '-' + col + ' > div:nth(' + num + ') > input').val("");
...
<table width="95%" border="0" cellspacing="0" cellpadding="5" align="center">
    <tr>
        <td class="infobox" colspan="2">
            ...
            <a id="filterLink" class="filterLink" href="checkpoint_summary" onclick="filterSelected();">Filter</a>
            <c:if test='${!empty param.filterValue}'>
                / <a class="filterLink" href="checkpoint_summary">Remove Filter</a>
            </c:if>
    ...
    <tr>
        ...
        <td><b>Included Types:</b> <div id="selectedIncc"></div><b>Excluded Types:</b> <div id="selectedExcc"></div></td>
        <td><b>Included State:</b> <div id="selectedInct"></div><b>Excluded State:</b> <div id="selectedExct"></div></td>
        <td colspan="4"></td>
    </tr>
    <tr>
        <td colspan="7"><a id="customFilterLink" class="filterLink" href="checkpoint_summary" onclick="doCustomFilter();">FILTER</a>
    ...
    <tr>
        ...
        <th class="subHeading">Issue Types</th>
        <th class="subHeading">State</th>
        <th colspan="4"></th>
    </tr>
    ...
    <tr>
        ...
        <td class="formattedCell" id="${rowStatus.index}-${colStatus.index}">
            <c:choose>
                <c:when test="${!empty col && rowStatus.index == 0 && colStatus.index == 0}">
                    <div><input></input></div>
                    <span class="deleteLink" onclick="addSelectedText('selectedIncm', 'selectedExcm', ${rowStatus.index}, ${colStatus.index}, 0);">Include</span>
                    <div><input></input></div>
                    <span class="deleteLink" onclick="addSelectedText('selectedExcm', 'selectedIncm', ${rowStatus.index}, ${colStatus.index}, 1);">Exclude</span>
                </c:when>
